...
- **Provider/Model 解耦（v2）**：`ProviderConfig.models[]` + `Config.default_model`（`provider/model` canonical，serde default 零迁移——`models_or_legacy`/`resolve_default_model` 回退旧单 model）；运行期 `provider_cfg.model` = 活动模型（下游零改动）；`/model` 全量分组选择（`apply_model_switch(provider,model)` 重建 client 不要求重输 key，落盘 default_model）；Setup Model 步空格多选；config.toml 保持 `[[providers]]` 数组形态（**不**嵌套表，用户确认）
