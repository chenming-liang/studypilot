...
- **Model Role（Task→Role→Configured Model，docs §四.1）**：`ModelRole{Fast,Balanced,Reasoning}` + config.toml `[roles]` 表（`BTreeMap<String,String>`，canonical `provider/model`）；`Config::resolve_model(role)`（roles → 回退 default_model → 报错）；TUI `App.roles`/`resolve_role_cfg`/`role_client`（未配置 role 复用当前 client 零额外请求）；任务入口映射 Fast=导入/概念/大纲/复习地图/闪卡/出题、Balanced=普通问答/复习追问、Reasoning=复习小结/简答批改；`/model fast|balanced|reasoning` 切角色模型（未配置明确报错），/model 落盘保留 roles
