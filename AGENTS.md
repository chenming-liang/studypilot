...
- [x] 收口会话㉙：**首屏 UX 补洞：重启恢复不空白 + last course 持久化（onboarding 续）**——用户实测截图：有课程时启动首屏仍大片空白（Welcome 只在 0 courses 触发、启动从不弹 Summary、`App::new` 硬编码 course="rust" 无恢复逻辑）。修复：① 四场景区分（first launch→Welcome / 创建→Summary / 切换→Summary / **重启恢复→轻量 context 卡**），`push_startup_cards` 统一分发；② **last course 持久化**（data/last_course.json 仿 budget.json，无 schema）：切换/创建 `persist_last_course` 写盘、启动 `restore_last_course` 恢复（课程已删忽略→落第一门）；③ **Course Summary 三态自适应**：0 材料 / 有材料无 session（Your course is ready + Get started 动作行）/ 有 session（`**Continue:** {标题}` + `/review` · `/review-map` · `/outline` · `/import`），统计只作上下文不作 KPI；④ Welcome 标题 `# Welcome to StudyPilot`。测试 +3（重启 context 卡/Get started vs Continue 变体/last_course 持久化往返），workspace 99 全绿（+1 ignored 首屏预览 `preview_screen`）；首屏渲染经 markdown 管线实测核对。**注意**：`push_welcome_if_fresh` 已改名为 `push_startup_cards`（0 courses 分支不变）
- [x] 收口会话㉚：**Home / Course / Session 三级 workspace（Launchpad 化，feature/onboarding-cards 分支）**——首屏不再是"聊天流里的卡片"，而是明确三个不同 UI state。① `App.workspace` 枚举（Home→Course→Session），启动一律进 Home（启动 ≠ 恢复工作区）；② **HOME**：空库原生渲染 Welcome（复用 cards::welcome_guide）、有库显示 Continue learning（最近 session）+ Your courses + New Course；**无聊天历史/无输入框/无 tool trace**；③ **COURSE**：统计（notes/concepts/weak 异步查库缓存）+ Continue / New conversation / Review Map / Outline / Import 动作 + Recent learning（Enter 直开 session）；④ **SESSION**：原聊天工作区原样保留；⑤ 导航：Home/Course 光标 ↑↓+Enter，**Esc 逐级回退**（Session→Course→Home，Home 才退出；Ctrl+C/Q 仍直接退）；Home/Course 任何键入打开命令面板；命令执行经 submit 自动切回 Session；鼠标滚动/拖选仅 Session 生效；⑥ 侧栏 session 挂当前课程下（不再平级 RECENT）、header 面包屑 Home│Course│Session；⑦ **导航状态不入 session history**（纯 UI state）；⑧ last course 持久化保留但不再推首屏卡。改动：app/mod.rs（Workspace/导航/weak_stats/continue_session）、keys.rs（workspace 路由）、ui.rs（draw_home/draw_course/面包屑/侧栏嵌套/输入提示）、commands.rs（submit 切入 Session）、outline/import/refresh handler 入口切入 Session。测试 +6（Home 启动/创建进 Course/重启 Home/Continue 直达/课程切换/Esc 逐级回退/无 session 不伪造 Continue），workspace 182 全绿（+2 ignored preview）。**注意**：`push_startup_cards` 现只恢复上下文不再 push 卡
- [x] 收口会话㉛：**Home Launchpad 视觉改版**（feature/onboarding-cards 分支续）——功能正确但"像 CLI 菜单"：① **Home 全宽**：draw() 里 Home 不画 28 列课程侧栏（Course/Session 才恢复），内容居中偏上（水平让出 1/6 宽、顶部两空行），不再挤左上角；② **视觉层级**：StudyPilot 标题 → Continue learning 主入口（course · title + **Last studied** 相对时间 + Continue →）→ Your courses 内容项 → + New Course；③ **去 CLI 化**：Home 主视觉删除 `/course -new` 等命令文案，改用 `+ New Course` 光标项（Enter 开创建向导，`home_cursor_count` 含此项、空库=1）；选中指示 `▶` → 轻量 `●`；④ **Last studied**：`SessionMeta` 只读加 `created_at`（sessions.created_at，非 schema 变更），`last_studied()` 算相对时间（min/hr/d）；⑤ **Session title**：Continue 卡显示人类可读 title、不显示 session ID；⑥ 底部 hint 轻量化（`↑↓ navigate · Enter open · / commands`）。`home_lines()`/`draw_course` 提纯函数便于单测。测试 +4（home_lines 层级/Continue 标题无 ID/空库 Welcome/New Course 向导），workspace 186 全绿（+1 ignored preview）。**注意**：cards::welcome_guide 已 `#[cfg(test)]`（Home 原生渲染 Welcome，不再用卡）
- [x] 收口会话㉜：**Home visual refinement（纯视觉 IA，不动 Course/Session/DB）**——用户截图：还是"几个文本块排一起"。修复：① **删除正文重复 StudyPilot**（header 已有）；② **Continue learning 第一视觉焦点**：标题 accent+bold、course·title 主内容行、Last studied 辅助、Continue → 动作，用空行节奏分层；③ **无意义 session 标题 fallback**（`session_display_title`：空/<3 字符/问候词 你好·hello·hi·test→`Recent conversation`，不改 DB）；④ **Your courses 去 picker 化**：课程名独立一行（主要信息）+ 统计缩进副行，不用 `▶`；⑤ **居中偏上**：内容列 `(w*3/4).clamp(20,76)` 水平居中，不再缩左上；⑥ 三级层级 PRIMARY Continue / SECONDARY courses / TERTIARY New Course。改动：仅 `ui.rs`（draw_home 居中 + home_lines 重排 + session_display_title）。测试 +1（无意义标题 fallback）、更新 2（去 StudyPilot/课程双行），workspace 全绿（+1 ignored preview_home 三态文本核对）。**遗留**：Home/Course 鼠标点击仍未支持（键盘 ↑↓ Enter 可导航）
- [x] 收口会话㉝：**workspace 课程上下文脱节修复（state 隔离根因）**——用户实测：Home `/course -new csapp` 后 Home 显示 csapp，但进入新课程 Session 仍显示 rust。**根因（实证复现）**：`app.course`（权威当前课程）与 `home_cursor`（Home 列表光标）是两份 state——创建/切换课程时 `on_course_managed` 只更新 `app.course`，`home_cursor` 停在原位（指向 rust 索引），用户按 Enter 走 `home_activate`→`courses[home_cursor]` → 进了旧课。**修复（单一事实源）**：`app.course` 为唯一权威，`home_cursor` 从它派生——新增 `sync_home_cursor_to_course()`，挂到五处课程上下文变更点：`on_course_managed`（创建/删除）、`handle_course_command` Switch/SwitchById、`CoursesRefreshed` 事件（导入新建课）、`push_startup_cards`（重启恢复）、`go_back` Course→Home（此前硬编码 `home_cursor=0` 同样会进错课）。回归测试 +4（直接命令创建/已有旧 session 前提/Home 向导创建后回退重进/不移动光标直接 Enter——后者精确复现报告场景，修复前断言失败、修复后通过）。workspace 191 全绿（+4 course_context_tests）
- [x] 收口会话㉞：**New Course 对话框直接叠在 Home + 建完直达 Course 页**——用户质疑"为什么建课必须进 Session 且看不到 New conversation/Outline"。修复（方案 A）：① `open_course_creation_wizard` 删掉 `enter_session_workspace()`——向导本就是模态覆盖层（draw_wizard 任意 workspace 上绘制、handle_wizard_key 有 is_some 守卫），对话框直接叠在 Home 弹出；② 新增 `App.pending_course_enter` 标志：从 Home 发起（workspace==Home）时置位，`on_course_managed` 收到非 all 的 switch_to 且标志为真 → `enter_course_workspace(csapp)` 直达新课程 Course 页（New conversation/Outline/Review Map/Import 立即可见），并跳过摘要卡（Course 页原生展示统计）；③ 从 Session `/course -new` 不置位，保持原地不被拽走；④ palette 的 CreateCourse Prompt 改走 `open_course_creation_wizard` 共用入口；⑤ Esc 取消/空名时清标志防残留。测试：更新 2、新增 1（Session 内建课不切换 workspace），workspace 192 全绿
