...
- **UI/颜色重构（纯 token）**：正文=终端默认前景、主视觉=Primary 蓝、移除紫色（SECONDARY=PRIMARY）、新增 WARNING 黄；Tool 活动行 ⟳muted/✓绿/✗红+正文默认；Review Map Section 恒蓝+mark 语义色；◌ 活动 header=muted、● Ready=绿；Color::Gray 统一 MUTED；**Runtime Status 语义色（`theme::StatusKind` + `status_color` 集中映射）**——Processing=暖橙 0xE8A35C、Tool=青、Success=绿、Warning=黄、Error=红、Idle=灰，`status_label` 不再用蓝色主视觉
- **Model Role（Task→Role→Configured Model，docs §四.1）**：`ModelRole{Fast,Balanced,Reasoning}` + config.toml `[roles]` 表（`BTreeMap<String,String>`，canonical `provider/model`）；`Config::resolve_model(role)`（roles → 回退 default_model → 报错）；TUI `App.roles`/`resolve_role_cfg`/`role_client`（未配置 role 复用当前 client 零额外请求）；**任务入口映射（用户 2026-09-04 调整）**——Fast=导入/概念抽取/大纲/复习地图/闪卡、Balanced=普通问答/复习追问/正式 Review 出题/复习小结建议/简单答案评价、Reasoning=复杂简答批改/错因分析/深层概念分析/跨材料综合/复杂代码问题；**角色配置 = UI（/model 无参或 /model fast|balanced|reasoning）**——面板顶部角色行 Enter 进该角色模型选择、Enter 绑定落盘 `[roles]`（`apply_role_assign` 不改当前对话模型）**且不关闭面板——返回角色面板光标落回刚配置行可连续配置其他角色**；**搜索过滤（fzf 风格）**——`App.input` 即搜索栏（take_input_for_overlay 备份清空，同 CommandPalette），编辑键经 `sync_picker_filter`/`refilter` 实时 substring 过滤（匹配 provider/模型名/角色名，空输入恢复全量），导航/Enter 基于 `filtered` 索引；`/model provider/model` 直接切换保留；**未配置角色的任务自动用用户 /model 选择的当前模型**（fallback），/model 落盘保留 roles
...
- **产品化（productization，Phase 3-6）**：pricing 可选化（`ProviderConfig.price_*` → `Option<f64>`、`known_pricing()`，未知模型 Cost tracking unavailable 可正常运行）；Runtime Config（`Config::save`/`runtime_path`，main 优先 ~/.studypilot/config.toml 回退 cwd、缺失/损坏不 crash 用 default，/model 切换落盘）；Model Registry（`providers::registry`，14 厂商预设 + custom_provider，见下条）；AI Setup（`App::ai_needs_setup()` Home 引导 + `/test` 连接测试）
- **多 Provider/Model（2026-09-04 扩展）**：registry 从 3 家扩到 **14 家**——DeepSeek/Qwen/GLM/Kimi/MiniMax/Doubao/Hunyuan/ERNIE/OpenAI/Anthropic/Gemini/Grok/OpenRouter/Ollama，每厂商 2~6 个主流模型（`ProviderPreset` 增 `endpoint` 字段，`provider_from_preset` 直接读预设不再硬编码 match）；**协议分流**：除 Anthropic（Messages API：`x-api-key` + `anthropic-version` header、system 抽顶层、tool→tool_result block、`input_tokens/output_tokens`）外全走统一 OpenAI-compatible `chat/completions`——Gemini 用官方 OpenAI 兼容端点、OpenRouter 模型名带 `/`（`split_canonical` 用 split_once 只拆首个 `/`，天然兼容）、Ollama 本地免 key（`is_keyless_local` 跳过 key 检查）；`parse_anthropic_response` 纯函数解析；测试 +7（厂商齐全/多模型+endpoint/OpenRouter 斜杠/anthropic body 映射/anthropic 响应解析/qwen+anthropic 预设形态/setup 断言更新）
