...
| 角色 | 负责的任务 | 特点 |
|---|---|---|
| `fast` | 导入、概念抽取、大纲、复习地图、闪卡 | 大量、结构化、低难度，追求速度，不开启思考 |
| `balanced` | 普通问答、复习追问、正式出题、复习小结、简单答案评价 | 默认学习，追求响应快，不开启思考 |
| `reasoning` | 复杂简答批改、错因分析、深层概念分析、跨材料综合、复杂代码问题 | 高难度推理，开启思考保证质量 |

> 导入材料这类流程较机械、概念抽取/大纲/闪卡等高频任务，建议配给 `fast` 角色一个速度快、价格低的模型（如 `deepseek-v4-flash`），能明显提升体验。
