...
导入的资料会自动抽取为知识点，并按主题组织成「章节 → 概念」的结构。每个知识点标记掌握状态：已掌握（✓）、需巩固（△）、未复习（○），让"学了什么、哪里薄弱"一目了然。地图可以导出为 Markdown 文件（`Outline → Export`），作为考试前的复习提纲随身带走。
...
笔记、会话、学习历史全部保存在本地 SQLite 数据库中，不依赖任何云端存储。删除一门课程即从本地移除，数据不留存在服务器上。重复导入同一份资料也不会产生重复内容（按内容去重）。
